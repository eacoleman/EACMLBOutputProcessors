...
void MassFit::calib(char fn[100],char tag[20]){
    float massPoints[5] = { 169.5, 171.5, 172.5, 173.5, 175.5};
...
void MassFit::setup()
{
    ...
    cout<<"Initializing more helper variables"<<endl;
    ...
    cout<<"Initializing ROO helper variables"<<endl;
...
double MassFit::fitPoint(int i)
{
    ...
    if (myFitResults_all)  {
        ...
        if (bkgsyst) {
            cout<<" - deleting more things"<<endl;
            delete pdffit;
    ...
    cout<<" - we're at the end now"<<endl;
...
pair<double,double> MassFit::findMinFake(bool all, int pointsToUse) {
    ...
    gr->SetName("gr");
    gr->Fit("pol2","Q","",mcSignalTemplMass[min], mcSignalTemplMass[max]);
    gr->GetXaxis()->SetTitle("MLBWA mass [GeV]");
    gr->GetYaxis()->SetTitle("-log (L/L_{max})");
    gr->GetYaxis()->SetTitleOffset(1.25);
...
void MassFit::do_toys(int n_exp, int templateToUse)
{
    ...
    for (int i=0;i<n_exp;i++){
        ...
        if (stat !=0) {
            cout << "Too many consecutive failues\n";
            exit(1);
        }
